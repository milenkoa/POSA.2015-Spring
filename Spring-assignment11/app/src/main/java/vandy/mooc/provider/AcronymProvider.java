            ...
            case ACRONYMS:
                ...
                try {
                    ...
                    for (ContentValues cvs : contentValues) {
                        final long id = db.insert(AcronymContract.AcronymEntry.TABLE_NAME, null, cvs);
                        if (id != -1) {
                            returnCount++;
                        }
                    }
    ...
    public Cursor query(Uri uri,
        ...
        switch (sUriMatcher.match(uri)) {
            case ACRONYMS:
                ...
                SQLiteDatabase db1 =
                        mOpenHelper.getWritableDatabase();
                retCursor = db1.query(AcronymContract.AcronymEntry.TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder );
                break;
            case ACRONYM:
                ...
                //retCursor = null;
                SQLiteDatabase db2 = mOpenHelper.getReadableDatabase();
                retCursor = db2.query(AcronymContract.AcronymEntry.TABLE_NAME, projection, rowId, selectionArgs, null, null, sortOrder);
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: "
                        + uri);
