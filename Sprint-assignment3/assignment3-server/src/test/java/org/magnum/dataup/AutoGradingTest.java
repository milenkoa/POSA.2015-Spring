...
public class AutoGradingTest {
	...
	public void testAddVideoRating() throws Exception {
	    ...
	    received = videoSvc.rateVideo(received.getId(), 5);
	    received = videoSvc.rateVideo(received.getId(), 2);
	    received = videoSvc.rateVideo(received.getId(), 3);
		...
		assertTrue(received.getRating() == 3);
		assertTrue(received.getId() > 0);
		assertTrue(received.getDataUrl() != null);
	}
	
	@Rubric(
			value="Wrong rating value returns status 400",
			goal="The goal of this evaluation is to ensure that your Spring controller(s) "
					+ "properly handles wrong values for video rating",
			points=20.0,
			reference="This test is derived from the material in these videos: "
					+ "https://class.coursera.org/mobilecloud-001/lecture/61 "
					+ "https://class.coursera.org/mobilecloud-001/lecture/67 "
	...
	@Test
	public void testAddWrongVideoRating() throws Exception {
		Video received = videoSvc.addVideo(video);
	    
	    try{
	    	received = videoSvc.rateVideo(received.getId(), 10);
	    ...
	    try{
	    	received = videoSvc.rateVideo(received.getId(), -1);
