...
public class VideoController {
    ...
    */
     ...
     *
     * @param videoId
     *            Id of the Video to be downloaded.
     *
     ...
     *                Uploading Video.
     */
    public boolean downloadVideo(long videoId, String videoName) {

        Log.d(TAG, "downloadVideo() in");
        Response response = mVideoServiceProxy.getData(videoId);

        if (response != null) {
            Log.d(TAG, "downloadVideo() - storing to external directory");
            File result = VideoStorageUtils.storeVideoInExternalDirectory(mContext, response, videoName);

            if (result == null) {
                Log.d(TAG, "downloadVideo() failed");
                return false;
            }
            else {
                Log.d(TAG, "downloadVideo() successful");
            }
        }
        else {
            return false;
    ...
    public int rateVideo(long id, int rating) {
        Video v = mVideoServiceProxy.rateVideo(id, rating);
        return v.getRating();
    }
}
